    ...
    return (  
        ...
        <Menu.Items className={ mobile ? 'bg-neutral-grayish-blue bg-opacity-30 mt-4 rounded-lg shadow-md px-8' : 'absolute bg-primary-light-red top-12 left-0 rounded-lg shadow-md z-10'}>
            <ul className='flex flex-col gap-4 p-6 w-full lg:w-[190px]'>
                <Menu.Item as='li' className={ mobile ? 'opacity-90' : 'text-neutral-white hover:text-primary-very-dark-blue'} onClick={() => {setOpen(false)}}>Contact</Menu.Item>
                <Menu.Item as='li' className={ mobile ? 'opacity-90' : 'text-neutral-white hover:text-primary-very-dark-blue'} onClick={() => {setOpen(false)}}> Newsletter</Menu.Item>
                <Menu.Item as='li' className={ mobile ? 'opacity-90' : 'text-neutral-white hover:text-primary-very-dark-blue'} onClick={() => {setOpen(false)}}>LinkedIn</Menu.Item>
            </ul>
        </Menu.Items>
    </Menu>
    );
